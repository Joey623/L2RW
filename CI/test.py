...
parser.add_argument('--resume', '-r', default='flvireid_final_llcm_p8_n8_lr0.2_sgd_seed_0_final.pth', type=str,
    ...
    with torch.no_grad():
        ...
        with torch.no_grad():
            for idx, (img, _) in enumerate(gallery_loader):
                ...
                if args.tta:
                    global_feat_tta, feat_tta = model(torch.flip(img, dims=[3]))
...
if args.dataset == 'sysu':
    ...
    for trial in range(10):
        test_trial = trial + 1
        model_path = args.model_path + args.dataset + '/' + 'fl_regdb_p8_n8_lr0.2_sgd_seed_0_trial_{}.pth'.format(
            test_trial)
        checkpoint = torch.load(model_path)
        print('==> best epoch', checkpoint['epoch'])
        model.load_state_dict(checkpoint['net'])
