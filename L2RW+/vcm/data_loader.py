...
class VideoDataset_train(Dataset):
    ...
    def __init__(self, dataset, seq_len=6, sample='evenly', transform=None, Index=[]):
    ...
    def __getitem__(self, index):
        ...
        S = self.seq_len
        sample_clip = []
        frame_indices_img = list(range(num))
        if num < S:
            strip = list(range(num)) + [frame_indices_img[-1]] * (S - num)
            for s in range(S):
                pool = strip[s * 1:(s + 1) * 1]
                sample_clip.append(list(pool))
        else:
            inter_val = math.ceil(num / S)
            strip = list(range(num)) + [frame_indices_img[-1]] * (inter_val * S - num)
            for s in range(S):
        ...
        if self.sample == 'random':
            ...
            idx1 = np.random.choice(sample_clip.shape[1], sample_clip.shape[0])
            number = sample_clip[np.arange(len(sample_clip)), idx1]
            imgs = []
            for index in number:
                index = int(index)
                img_path = img_paths[index]
