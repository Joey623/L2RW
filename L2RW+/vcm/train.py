...
suffix = suffix + '_p{}_n{}_lr{}_{}_s{}_seed_{}'.format(args.batch_size, args.num_pos, args.lr, args.optim, args.seq_lenth, args.seed)
...
class PrototypeMemory(nn.Module):
    def __init__(self, class_num, feature_dim=2048, top_k=4):
...
if dataset == 'vcm':
    ...
    for index in range(1, client_num+1):
        ...
        sub_net.to(device)
        local_prototype = PrototypeMemory(class_num=500)
        local_prototype.to(device)
        net.append(sub_net)
        local_prototypes.append(local_prototype)

# only infrared to visible
queryloader = data.DataLoader(VideoDataset_test(test_set.query, seq_len=seq_lenth, sample='video_test', transform=transform_test),
...
for epoch in range(start_epoch, args.max_epoch):
    ...
    for index in range(len(train_set)):
        ...
        trainloader = data.DataLoader(VideoDataset_train(train_set[index].train, seq_len=seq_lenth, sample='video_train', transform=transform_train,Index=Index),
    ...
    if epoch == args.max_epoch - 1:
        ...
        if not os.path.exists(model_path):
            os.makedirs(model_path)
        torch.save(state, model_path + 'l2rw_vcm.pth')
