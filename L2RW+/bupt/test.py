...
def test(model, dataloader_query, dataloader_gallery, show=False, save_dir='', return_all=False, postfix=''):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (imgs, pids, cids, modals) in enumerate(dataloader_query):
            ...
            modal = modals[0]
            if modal == 0:
                _, feats = model(x_v=imgs)
            elif modal == 1:
                _, feats = model(x_r=imgs)
            else:
                continue
    ...
    for q_modal in (0, 1):
        for g_modal in (0, 1):
            ...
            tmp_cmc, tmp_ap, tmp_inp = evaluate(tmp_distance, tmp_qid, tmp_gid, opt)
            CMC.append(tmp_cmc * 100)
            MAP.append(tmp_ap * 100)
            MINP.append(tmp_inp * 100)
            if show:
                print_metrics(
                    tmp_cmc, tmp_ap, tmp_inp,
                    prefix='{:<3}->{:<3}:  '.format(MODALITY_[q_modal], MODALITY_[g_modal])
                )

    # evaluate (omni-modality)
    cmc, ap, inp = evaluate(distance, query_pids, gallery_pids, opt)
    CMC.append(cmc * 100)
    MAP.append(ap * 100)
    MINP.append(inp * 100)

    if show:
        print_metrics(cmc, ap, inp, prefix='AllModal:  ')
    ...
    if return_all:
        ...
        return cmc * 100, ap * 100, inp * 100


if __name__ == '__main__':
    os.environ['CUDA_VISIBLE_DEVICES'] = opt.gpus
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    model = embed_net(class_num=1074).to(device)
    model, _ = load_from_ckpt(model, 'net', opt.test_ckpt_path)
