...
class IdentitySampler(Sampler):
    def __init__(self, dataset, np, nt):
        self.np = np
        self.nt = nt
    ...
    def _get_final_idx(self):
        ...
        for index, (pid, _, _) in enumerate(self.dataset.fast_iteration()):
            index_dict[pid].append(index)

        pid2batch = defaultdict(list)
        for pid, idx_list in index_dict.items():
            if len(idx_list) >= self.nt:
                idx_copy = idx_list.copy()
                while len(idx_copy) >= self.nt:
                    sampled_index = random.sample(idx_copy, self.nt)
                    pid2batch[pid].append(sampled_index)
                    idx_copy = [i for i in idx_copy if i not in sampled_index]
