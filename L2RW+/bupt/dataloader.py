...
def get_dataloader(opt, mode, client_id, show=False):
...
class BUPTCampus_Dataset(Dataset):
    def __init__(self, data_root, mode, sample, seq_len, client_id, transform, random_flip=False, fake=False):
        """
        :param data_root:
        ...
        assert 0 <= client_id <= 6
        # training: clinet_id:1-6
        # testing: client_id:0
        ...
        self.client_data_info = self.split_client(client_id=client_id)
    ...
    def split_client(self, client_id):
        assert 0 <= client_id <= 6, 'The value of client_id should be between 0 and 6 in training, check the param.'
        client_data_info, client_pids = [], []
        for obj_id, modality, camera, tracklet_id in self.data_info:
            if CAMERA[camera] == client_id-1:
                client_data_info.append((obj_id, modality, camera, tracklet_id))
                client_pids.append(obj_id)
            else:
                continue
        self.client_pids = sorted(set(client_pids))
    ...
    def fast_iteration(self):
        if self.client_id >=1 and self.client_id <=6:
            iteration = [
            [self.pid2label[obj_id], MODALITY[modality], CAMERA[camera]]
            for (obj_id, modality, camera, tracklet_id) in self.client_data_info
            ]
        else:
            iteration = [
                [self.pid2label[obj_id], MODALITY[modality], CAMERA[camera]]
    ...
    def swap_half_frames(self, images_ir, images_rgb):
        T = images_ir.shape[0]
        indices = random.sample(range(T), T // 2)  # 随机选择 50% 的帧索引进行互换

        # 避免原地修改
        mixed_ir = images_ir.clone()
        mixed_rgb = images_rgb.clone()

        for idx in indices:
            temp = mixed_ir[idx].clone()
            mixed_ir[idx] = mixed_rgb[idx]
            mixed_rgb[idx] = temp

        return mixed_ir, mixed_rgb

    def __getitem__(self, index):
        # obj_id, modality, camera, tracklet_id = self.data_info[index]
        if self.mode == 'train':
            obj_id, modality, camera, tracklet_id = self.client_data_info[index]
        else:
            obj_id, modality, camera, tracklet_id = self.data_info[index]
        ...
        if self.mode in ('train', 'auxiliary'):
            ...
            # imgs = torch.cat((images_rgb, images_ir), dim=0)
            ...
            # images_ir, images_rgb = self.swap_half_frames(images_ir, images_rgb)
            # if self.random_flip and torch.rand(1) < 0.5:
            #     images_rgb = images_rgb.flip(-1)
            #     images_ir = images_ir.flip(-1)
                # imgs = imgs.flip(-1)
    ...
    def __len__(self):
        if self.mode in ('train', 'auxiliary') and 1 <= self.client_id <= 6:
            return len(self.client_data_info)
        else:
    ...
    def show_information(self):
        if self.mode in ('train', 'auxiliary'):
            ...
            factor = 1
        if self.mode == 'train' and self.client_id==1:
            print(
            ...
            print('================>')
            print('Client {} Information:'.format(self.client_id))
            print(
                f"Number of identities: {len(self.client_pids)}\n"
                f"Number of samples   : {len(self.client_data_info) * factor}"
            )
