    ...
    with torch.no_grad():
        ...
        with torch.no_grad():
            for idx, (img, _) in enumerate(gallery_loader):
                ...
                if args.tta:
                    global_feat_tta, feat_tta = model(torch.flip(img, dims=[3]), torch.flip(img, dims=[3]), modal=1)
    ...
    with torch.no_grad():
        ...
        with torch.no_grad():
            for idx, (img, _) in enumerate(query_loader):
                ...
                global_feat, feat = model(img)
                if args.tta:
                    global_feat_tta, feat_tta = model(torch.flip(img, dims=[3]), torch.flip(img, dims=[3]), modal=2)
                    global_feat += global_feat_tta
                    feat += feat_tta
...
if args.dataset == 'sysu':
    if len(args.resume) > 0:
        model_path = args.model_path + protocol + '/' + args.resume
