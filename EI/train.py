...
parser.add_argument('--protocol', default='R_L_to_S', help='R_S_to_L, R_L_to_S, L_S_to_R')
...
suffix = 'final'
suffix = suffix + '_{}_p{}_n{}_lr{}_{}_seed_{}'.format(protocol, args.batch_size, args.num_pos, args.lr, args.optim,
                                                       args.seed)
...
class PrototypeMemory(nn.Module):
    def __init__(self, class_num, feature_dim=2048, top_k=3):
...
def train(epoch, optimizer, net, args, server_weight, local_prototype, global_prototype):
    ...
    for batch_idx, (input1, labels) in enumerate(trainloader):
        ...
        if epoch == 0:
            ...
            # sup_loss.update(loss_sup.item(), input1.size(0))
        else:
            train_loss.update(loss.item(), input1.size(0))
            id_loss.update(loss_id.item(), input1.size(0))
            tri_loss.update(loss_tri.item(), input1.size(0))
            pro_loss.update(loss_pro.item(), input1.size(0))
            # sup_loss.update(loss_sup.item(), input1.size(0))
        # train_loss.update(loss.item(), input1.size(0))
        # id_loss.update(loss_id.item(), input1.size(0))
        # tri_loss.update(loss_tri.item(), input1.size(0))
        ...
        #                  'SLoss: {sup_loss.val:.4f} ({sup_loss.avg:.4f}) '
        #                  'Accu: {:.2f}'.format(
        #         epoch, batch_idx, len(trainloader), current_lr, base_lr,
        #         100. * correct / total, batch_time=batch_time,
        #         train_loss=train_loss, id_loss=id_loss, tri_loss=tri_loss, pro_loss=pro_loss, sup_loss=sup_loss))
        if batch_idx % 20 == 0:
            print('Epoch: [{}][{}/{}] '
                  'Time: {batch_time.val:.3f} ({batch_time.avg:.3f}) '
                  'lr:{:.6f} '
...
for epoch in range(start_epoch, args.max_epoch):
    ...
    for index in range(len(train_set)):
        ...
        w, rep_local_proto = train(epoch, client_optimizer[index], net=net[index], args=args,
                                   server_weight=server_weight, local_prototype=local_prototypes[index],
