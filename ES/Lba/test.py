...
parser.add_argument('--resume', '-r', default='R_L_to_S_lba_p8_n4_lr_0.1_seed_0_best.pth', type=str,
...
print('Test Protocol: ', args.protocol)


# feat= model(img, img, test_mode[1], use_cmalign=False)['feat4_p_norm']
    ...
    with torch.no_grad():
        ...
        with torch.no_grad():
            for idx, (img, _) in enumerate(gallery_loader):
                ...
                global_feat, feat = model(img, img, test_mode[0], use_cmalign=False)['feat4_p'], \
                model(img, img, test_mode[0], use_cmalign=False)['feat4_p_norm']
    ...
    with torch.no_grad():
        ...
        with torch.no_grad():
            for idx, (img, _) in enumerate(query_loader):
                img = Variable(img.to(device))
                global_feat, feat = model(img, img, test_mode[1], use_cmalign=False)['feat4_p'], \
                model(img, img, test_mode[1], use_cmalign=False)['feat4_p_norm']
                batch_num = img.size(0)
...
model = embed_net(args, num_classes)
model.to(device)
