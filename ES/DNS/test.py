...
parser.add_argument('--resume', '-r', default='R_S_to_L_dns_p6_n4_lr_0.2_seed_0_best.pth', type=str,
    ...
    with torch.no_grad():
        ...
        with torch.no_grad():
            for idx, (img, _) in enumerate(gallery_loader):
                ...
                if args.tta:
                    global_feat_tta, feat_tta = model(torch.flip(img, dims=[3]), torch.flip(img, dims=[3]),
                                                      modal=test_mode[0])
