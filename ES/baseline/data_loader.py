...
class VIReIDDataset(data.Dataset):
    def __init__(self, args, data_dir, protocol, colorIndex=None, thermalIndex=None):
        data_path = data_dir + protocol + '/'

        train_color_image = np.load(data_path + 'train_rgb.npy')
        self.train_color_label = np.load(data_path + 'train_rgb_label.npy')

        train_thermal_image = np.load(data_path + 'train_ir.npy')
        self.train_thermal_label = np.load(data_path + 'train_ir_label.npy')
...
class AllVIReIDDataset(data.Dataset):
    def __init__(self, args, data_dir1, data_dir2, data_dir3, protocol, colorIndex=None, thermalIndex=None):
        data_path1 = data_dir1 + protocol + '/'
        data_path2 = data_dir2 + protocol + '/'
        data_path3 = data_dir3 + protocol + '/'
        train_color_image1 = np.load(data_path1 + 'train_rgb.npy')
        train_color_image2 = np.load(data_path2 + 'train_rgb.npy')
        train_color_image3 = np.load(data_path3 + 'train_rgb.npy')
        train_color_image = np.concatenate((train_color_image1, train_color_image2, train_color_image3), axis=0)
        self.train_color_image = train_color_image

        train_color_label1 = np.load(data_path1 + 'train_rgb_label.npy')
        train_color_label2 = np.load(data_path2 + 'train_rgb_label.npy')
        train_color_label3 = np.load(data_path3 + 'train_rgb_label.npy')
        train_color_label = np.concatenate((train_color_label1, train_color_label2, train_color_label3), axis=0)
        self.train_color_label = train_color_label

        train_thermal_image1 = np.load(data_path1 + 'train_ir.npy')
        train_thermal_image2 = np.load(data_path2 + 'train_ir.npy')
        train_thermal_image3 = np.load(data_path3 + 'train_ir.npy')
        train_thermal_image = np.concatenate((train_thermal_image1, train_thermal_image2, train_thermal_image3), axis=0)
        self.train_thermal_image = train_thermal_image

        train_thermal_label1 = np.load(data_path1 + 'train_ir_label.npy')
        train_thermal_label2 = np.load(data_path2 + 'train_ir_label.npy')
        train_thermal_label3 = np.load(data_path3 + 'train_ir_label.npy')
        train_thermal_label = np.concatenate((train_thermal_label1, train_thermal_label2, train_thermal_label3), axis=0)
        self.train_thermal_label = train_thermal_label
...
def get_datasets(args, VIReIDDataset, MultiVIReIDDataset, AllVIReIDDataset):
    ...
    if len(data_name_list_train) == 1:
        data_dir = dataset_dir_mapping.get(data_name_list_train[0], '')
        train_set = VIReIDDataset(args, data_dir, args.protocol)
        ...
        data_dir1 = dataset_dir_mapping.get(data_name_list_train[0], '')
        data_dir2 = dataset_dir_mapping.get(data_name_list_train[1], '')
        data_dir3 = dataset_dir_mapping.get(data_name_list_train[2], '')
        train_set = AllVIReIDDataset(args, data_dir1, data_dir2, data_dir3, args.protocol)
    else:
        raise "Wrong number of datasets!"
