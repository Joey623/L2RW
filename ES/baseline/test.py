...
parser.add_argument('--resume', '-r', default='R_S_to_L_baseline_p8_n4_lr_0.1_seed_0_best.pth', type=str, help='resume from checkpoint')
...
if data_name_test[0] in ['SYSU']:
    if len(args.resume) > 0:
        if len(data_name_list_train)==1:
            ...
            print("Domain generalization testing...")
            model_path = args.model_path + args.protocol + '/' + args.resume
            print('==> Loading weights from checkpoint......')
            checkpoint = torch.load(model_path)
            print('==> best epoch', checkpoint['epoch'])
    ...
    # args.tvsearch=1: visible to infrared
    if args.tvsearch:
        ...
        cmc_att, mAP_att, mINP_att = eval_llcm(distmat_att, gall_label, query_label, gall_cam, query_cam)
    ...
    if args.tvsearch:
        print('Test LLCM, Visible to Thermal')
    else:
        print('Test LLCM, Thermal to Visible')
    print(
        'original feature:   Rank-1: {:.2%} | Rank-5: {:.2%} | Rank-10: {:.2%}| Rank-20: {:.2%}| mAP: {:.2%}| mINP: {:.2%}'.format(
            cmc[0], cmc[4], cmc[9], cmc[19], mAP, mINP))
    print(
    ...
    if args.tvsearch:
        print('Test RegDB, Visible to Thermal')
    else:
        print('Test RegDB, Thermal to Visible')
