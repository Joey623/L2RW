...
from utils import *
from loss import OriTripletLoss, TripletLoss_WRT, KLDivLoss
...
parser.add_argument('--method', default='caj', type=str,
...
def train(epoch):
    ...
    for batch_idx, (input10, input11, input2, label1, label2) in enumerate(trainloader):
        ...
        # update P
        train_loss.update(loss.item(), 3 * input10.size(0))
        id_loss.update(loss_id.item(), 3 * input10.size(0))
        tri_loss.update(loss_tri.item(), 3 * input10.size(0))
        total += labels.size(0)
